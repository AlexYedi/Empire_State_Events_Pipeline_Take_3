...
- **Now writing (2026-07-11):** the DoD/judge semantic fields (`dod_met`/`dod_waived`/`correction_rounds`) — via `/dod-close` → `.claude/hooks/dod-close.sh` → `.claude/.state/<session>.build_meta` → Stop-hook fold. Before this they were null in 100% of rows (the loop's missing writer). See `<definition_of_done>`.
</measurement_rigor_layer>
