...
- **PostHog projection: LIVE** (2026-07-11) — `POSTHOG_PROJECT_KEY` + `POSTHOG_HOST` are set in `.env` (key `phc_uxaa…`, distinct from the connected MCP's `gtm_os_hub` project), so the Stop hook's `/capture/` branch fires. ⚠️ Confirm the destination PostHog project is the intended Empire State surface (the `.env` key is not the MCP-visible one) before relying on the dashboard.
