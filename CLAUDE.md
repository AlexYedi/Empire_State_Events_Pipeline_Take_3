...
- **Container rule (added 2026-09-18):** a parked / deferred / decision thought is filed to its home (per `.claude/references/linear-convention.md` §Container rule) **in the same turn** it is written anywhere else, and the file carries the Linear ID or Notion link. No new "awaiting decision" / "revisit after" text without a pointer.
...
1. **Spec artifact before code** — a PRD following **`.claude/references/prd-template.md`** (the canonical build-elevating shape: PR-FAQ head → alternatives-considered → leverage/systems check → inline pre-mortem → success + eval with a value-action-registry row → judge-gradeable acceptance criteria → dated decision log), authored in **ChatPRD**, mirrored to **Notion** (the source of truth for artifacts), created *before* the build where feasible (guards against retroactive cargo-cult PRDs). **Scope ruling (YED-129, 2026-09-18): product = PRD, infra = spec** — anything with a user/audience gets the ChatPRD→Notion PRD; pure infra/modeling (spine, migrations, hooks, judge plumbing, graph tooling) is specified by an ADR or an in-repo reference and needs no ChatPRD doc. The bar is **depth of reasoning, not length** — comprehensive on problem/alternatives/failure/decisions, implementation precision deliberately deferred as flagged build-time detail; held to ≤~1 page + decision log (creep is the rule-beating signal). Linear wiring per **`.claude/references/linear-convention.md`**.
...
4. **The build-quality judge ran on this build, in this session** (N = the build's own session; live since the eval layer shipped). **"Advisory" means the verdict doesn't gate — not that the run is optional.** A waiver of this item must say either *no gradable artifact* or name the Linear issue that holds the make-up run; `dod-close.sh` rejects an item-4 waiver (labelled `judge`, `item 4`, or `4`) whose reason names neither (YED-201 Fix 1A, ruled 2026-09-19, after 8 "advisory / next session" waivers went unredeemed). It's a guard, not a proof: it enforces the *format* (`judge: <reason>` with a pointer) but can't verify the named issue really holds the make-up run, so `/rigor-review` recounts the class weekly. The measurement system's health is a *precondition of the gate*, not a separate dashboard (prevents silent metric rot).

**Discipline:** keep this at ≤3 (+1) items. If it creeps toward 7, *shrink it* — growth is the rule-beating signal. The agent must surface this checklist (met / waived-with-reason per item) before closing a non-trivial build; it never hard-blocks (it informs). **To close (wired 2026-07-11 — this is what makes the gate leave a trace):** run `/dod-close` (or call `.claude/hooks/dod-close.sh --dod-met <bool> --dod-waived <bool> --correction-rounds <N> [--reason "item: why"]`) — it records met/waived/correction-rounds to `.claude/.state/$CLAUDE_CODE_SESSION_ID.build_meta` (the Stop hook folds them into the `build_session` telemetry row) and logs any waiver reason to `.claude/artifacts/dod-waivers.jsonl`. Before this, the DoD gate was surfaced but never written — the three semantic fields were null in 100% of telemetry rows.

**Promotion path:** piloted here in Empire State first; promote to canonical `~/Documents/GitHub/alex-agents-skills/Me/canonical-claude-md.md` (user-scope, inherits everywhere) once it has proven it changes behavior without becoming a drag — the YED-29/30 project-scope-first-then-promote pattern. Plan of record: **`.claude/references/roadmap.md`** (durable, version-controlled; the former `~/.claude/plans/my-linkedin-on-the-scalable-acorn.md` was machine-local and is retired).
</definition_of_done>

<measurement_rigor_layer>
**Build-rigor + measurement/eval/observability layer (added 2026-06-25/26).** Cures the diagnosed root cause (Shifting-the-Burden: rigor lived in optional docs/tools, not the execution path). Lean stack: Notion + PostHog + Empire State Hub + ChatPRD — NO Langfuse/gtm-os (those were tombstoned; do not reintroduce). North-star = acted-on value (outcome vs each artifact's assigned goal, trended). **Supabase tombstone re-scoped 2026-06-28 (ratified by Alex):** the ban applies to the *measurement/eval layer only* — do NOT reintroduce Supabase as a measurement/observability store. Supabase IS the sanctioned **system of record for the Market-Intelligence Engine** (the lens-agnostic MI/research engine; Job-Search + Content cores) — Supabase org `A.Yedi`, project `empire state ai` (ref `oicikjyzmxqfomrrqkvf`), accessed via **REST API** with `SUPABASE_API_KEY` from `.env` — **NOT** the Supabase MCP (that connector is on a different account, `Same Old Expressions`/GTM_OS — never use it for Empire State). See `.claude/references/market-intel-spine.md` (how the graph works now) + the roadmap `.claude/references/roadmap.md` (the former `~/.claude/plans/where-do-we-stand-sunny-puzzle.md` was machine-local and is retired). **Data-layer DECISIONS (why it's shaped this way) live in `docs/adr/` — read the relevant ADR before changing consolidation, the hubs' data access, the account boundary, or the event/intelligence model; reversing an ADR means writing a new one, not editing it. The reusable "change a live system safely" method is `docs/migration-playbook.md`.** The MI graph was consolidated onto this project (Empire prod DB, `oicikjyzmxqfomrrqkvf`) — the gtm signal spine (`abkvgihlbwfloentugtd`, other account) was decommissioned (YED-135 Done); the gtm-os-hub learning plane (`nnywrmetdoixdbevvsvf`) stays a separate GTM-owned project by design (YED-130).
- **Plan of record:** `.claude/references/roadmap.md` (durable master; retired the machine-local `~/.claude/plans/my-linkedin-on-the-scalable-acorn.md`). **Linear:** project "Empire State — Build-Rigor & Measurement Layer" (YED-87…94, `cycle-1`). **PRD:** ChatPRD `61bb864f…` + Notion Project Ideas `38ad3699…`. **Learnings/gotchas:** `.claude/notes/measurement-layer-learnings.md` (read before extending — it has the PostHog/OTEL/.env/transcript gotchas).
- **Live pieces:** the DoD gate (above) · signal scanners `/scan-trends`,`/scan-roles`,`/scan-voices` (skills `{trend,voice,role}-radar`) · telemetry Stop hook `.claude/hooks/build-session-emit.sh` (contract: `.claude/references/build-session-contract.md`) · the build-quality judge `/judge-build` + `.claude/evals/` (**rubric `build-quality@3` live** 2026-07-17; the ≥20-run-@-≥80% calibration gate **CROSSED** → judge **provisional-trusted** (gates new/independent builds, advisory on self-produced work); now a **cross-provider Sonnet + Gemini quorum** — adapter `.claude/hooks/gemini-judge.sh`, spec `.claude/references/cross-provider-judge.md`; agree→auto, disagree→human tiebreak (fail-safe FLAG in autonomous mode)) · **goal-tagging** (`Goal`/`Target` on Content Drafts; `content-patterns/goal-tagging.md`) + **outcome capture** `/tag-outcome` (`Outcome`/`Outcome Value`/`Outcome Date`) = the acted-on-value north-star · **weekly learning loop** `/rigor-review` + `.claude/evals/correction-recurrence.md` · the **value-action registry** `.claude/references/value-action-registry.md` (no orphan metrics; every metric has {threshold→action→surface}).
...
- **PostHog projection: LIVE** (2026-07-11; repointed to a dedicated project 2026-07-30) — canonical var set (same in both repos' env files): `POSTHOG_PROJECT_TOKEN` (`phc_`, capture) · `POSTHOG_PERSONAL_API_KEY` (`phx_`, query/read — must be scoped to the project + `query:read`) · `POSTHOG_PROJECT_API_KEY` (all-access spare) · `POSTHOG_PROJECT_ID=524367` · `POSTHOG_HOST`. Stop hook + judge emitter read `POSTHOG_PROJECT_TOKEN` for `/capture/`; the hub dashboard reads with the personal key. Project **524367 (`empire_state_events`) is the dedicated Empire State surface** for build-rigor telemetry — both repos point here (pipeline writes, hub reads); the separate `466893 (empire state hub)` project is reserved for future hub *web* analytics, NOT this loop. Verified 2026-07-30: capture 200, read 200 (13 build_session + 54 judge_run projected). ⚠️ Read path needs the `phx_` key scoped to 524367 (a project-secret `phs_` key is rejected by the query API).
- **Shipped 2026-07-11 → 07-17 (rigor-loop closure + judge hardening):** the DoD/judge semantic **writer** `/dod-close` → `.claude/hooks/dod-close.sh` → `.claude/.state/<session>.build_meta` → Stop-hook fold (telemetry now carries non-null `dod_met`/`dod_waived`/`correction_rounds` — the loop's missing writer) · first `/rigor-review` runs (seeded `.claude/evals/correction-recurrence.md`) · judge calibration **crossed the gate → provisional-trusted** · **`build-quality@2` then `@3`** (`build-quality-v2.md`/`-v3.md`: dangling-ref ≤0.60 + command-skeleton ≤0.35 + **composite confidence-honesty cap ≤0.65**) · the **command-orchestration convention** (`.claude/references/command-orchestration-convention.md`) · the **cross-provider Gemini judge** (adapter + `cross-provider-judge.md`; Approach-A backfill = 83% Gemini-vs-Alex / 94% Gemini-vs-Haiku, Approach-B started). Grounding-reconciliation closed 2026-09-18: the cross-provider judge's spec is `cross-provider-judge.md` (infra = spec, YED-129); no ChatPRD mirror owed.
</measurement_rigor_layer>
...
direct MCP writes to Notion and HubSpot. No middleware, no n8n. 
**(Updated 2026-08-07 — see `.claude/references/roadmap.md`, the live plan of record.** The "no Supabase" 
rule held for the base event pipeline, but the later **Market-Intelligence Engine uses Supabase as its 
system of record** — REST, `SUPABASE_API_KEY`, never the MCP. "No Supabase" now applies only to the base 
pipeline + the measurement/eval layer. This `<project_architecture>` section describes the original Q2-2026 
pipeline; the current program shape — MI Engine (M1/M2 done, M3 next) · empire-state-hub (canonical Hub) · 
Build-Rigor layer (closed) · Content & Voice — lives in `roadmap.md`.)**
...
13. **Claude design is the default visual generator (updated 2026-08-07 — supersedes the Gamma default).** Two lanes, no app-harness in either: **(a) structured / label-dense visuals** (diagrams, matrices, stat cards, timelines, carousels) are authored by Claude as **self-contained HTML/SVG** and published via the Artifact tool → export to 4:5 PDF/PNG (⌘P→Save-as-PDF gives a LinkedIn carousel). Claude renders exactly what's authored — no re-interpretation, which is what garbled dense labels in Gamma/Canva. **(b) Designy / pictorial imagery** (conceptual, editorial, illustrative, photographic) → **Gemini** (Alex's subscription; frontier flexibility, not a constrained app). **Gamma is REMOVED** (ripped out 2026-08-07 — the app re-interpreted content and broke labels); **Canva is vestigial**. Canonical spec: `.claude/skills/content-patterns/visual-briefs.md`. Proof: the AI Demo Night consolidation carousel.
