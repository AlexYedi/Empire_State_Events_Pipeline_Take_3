...
- **PostHog projection: LIVE** (2026-07-11; repointed to a dedicated project 2026-07-22) — canonical var set (same in both repos' env files): `POSTHOG_PROJECT_TOKEN` (`phc_`, capture) · `POSTHOG_PERSONAL_API_KEY` (`phx_`, query/read — needs `query:read` scope) · `POSTHOG_PROJECT_API_KEY` (`phs_`, project-secret — NOT accepted by the query API, verified 2026-07-22; reads use the personal key only) · `POSTHOG_PROJECT_ID=524367` · `POSTHOG_HOST`. The Stop hook + judge emitter read `POSTHOG_PROJECT_TOKEN` from `.env` for the `/capture/` branch; the hub dashboard reads with the personal/project key. Project **524367 is now dedicated to Empire State** (no longer the `gtm_os_hub` MCP project) — the earlier destination-ambiguity ⚠️ is resolved.
- **Shipped 2026-07-11 → 07-17 (rigor-loop closure + judge hardening):** the DoD/judge semantic **writer** `/dod-close` → `.claude/hooks/dod-close.sh` → `.claude/.state/<session>.build_meta` → Stop-hook fold (telemetry now carries non-null `dod_met`/`dod_waived`/`correction_rounds` — the loop's missing writer) · first `/rigor-review` runs (seeded `.claude/evals/correction-recurrence.md`) · judge calibration **crossed the gate → provisional-trusted** · **`build-quality@2` then `@3`** (`build-quality-v2.md`/`-v3.md`: dangling-ref ≤0.60 + command-skeleton ≤0.35 + **composite confidence-honesty cap ≤0.65**) · the **command-orchestration convention** (`.claude/references/command-orchestration-convention.md`) · the **cross-provider Gemini judge** (adapter + `cross-provider-judge.md`; Approach-A backfill = 83% Gemini-vs-Alex / 94% Gemini-vs-Haiku, Approach-B started). ✅ **Grounding-reconciliation complete (2026-07-21):** dual-judge wired into `/judge-build` + dangling-ref mechanized (`check-refs.sh`/`quorum-merge.sh`, PR #18); durable-spec PRD published to ChatPRD ("Empire State" project) + Notion (`3a4d3699…`, under the Measurement-Layer PRD); YED-109 reconciled.
</measurement_rigor_layer>
...
### Git — branch-first for non-trivial builds (added 2026-07-17)
**Scope: branch where it makes sense — the DoD's non-trivial line, not everything.**
- **Non-trivial builds** (skills, commands, hooks, agents, rubrics, `.claude/references/`, schema/data-contract changes, approved-plan work, anything hard to reverse) → **branch → PR → merge**, never a direct commit to `main`. As a Claude session: `git checkout -b build/<short-desc>` *before* committing, then open a PR.
- **Trivial churn** (telemetry `build-sessions.jsonl`, eval logs, `.claude/notes/`, docs, `CLAUDE.md` maintenance) → commit to `main` as normal.
- **Enforced by** a local `pre-commit` guard (`.git/hooks/pre-commit`, installed 2026-07-17) that **blocks** commits touching the protected build surface (`.claude/{skills,commands,hooks,agents}`, `evals/rubrics/`, `references/`) while on `main`; churn passes. It's **local-only (not version-controlled)** — a fresh clone won't have it, so the *convention here* is the durable part; re-install the guard per machine. Intentional main-commit override: `git commit --no-verify`.
- **Why:** the prior habit committed substantial builds straight to `origin/main` with no review (via GitHub Desktop). This restores review-before-merge for the work that warrants it without PR-ing every telemetry append. Branch→PR→merge already works here (e.g. PR #17).
