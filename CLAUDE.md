...
1. **Spec artifact before code.** **Product** builds (anything with a user or audience) owe a PRD following **`.claude/references/prd-template.md`**, authored in **ChatPRD** and mirrored to **Notion**, created *before* the build. **Infra/modeling** builds (spine, migrations, hooks, judge plumbing, graph tooling, ops hygiene) owe an **ADR or an in-repo reference** instead — and that **SATISFIES this item; it is not a waiver** (scope ruling YED-129, 2026-09-18; the wording tightened 2026-09-21 after 7 of 12 waivers in one week were infra builds all citing that same ruling — a gate that manufactures paperwork for a decided case trains people to waive). **A waiver here means one thing only: the code was written with no spec of any kind in front of it.** The bar is **depth of reasoning, not length** — comprehensive on problem/alternatives/failure/decisions, implementation precision deliberately deferred as flagged build-time detail; held to ≤~1 page + decision log (creep is the rule-beating signal). Linear wiring per **`.claude/references/linear-convention.md`**.
