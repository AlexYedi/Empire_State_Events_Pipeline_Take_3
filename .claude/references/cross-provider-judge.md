...
Both judges score all 5 `build-quality@2` criteria, BUT their votes are **weighted by domain competence**:
...
- `diagnostics` — shared.
Gemini gets the SAME `judge-system.md` + `build-quality@2` + per-artifact spec/context the Claude judge gets (apples-to-apples), plus a house-context primer for the convention criteria.
