...
| OpenAI key is saved as `OpenAI_KEY`, not the standard `OPENAI_API_KEY` | how it was first added to `.env` | `openai_judge.py` reads either name; rename when convenient. Keep auto-recharge OFF (prepaid credit is the true hard cap) and data sharing OFF; never join the free-daily-tokens program (it opts into training) | 2026-09-19 |
| Judge seats refuse gitignored / symlinked / out-of-repo evidence (exit 3, no override) | `judge_lib.privacy_guard`: a public repo must never leak a private ref to a provider | pass ad-hoc spec text with `--context`; spec FILES must be tracked and inside the repo. In a worktree the private refs are symlinks, so they are refused by construction | 2026-09-19 |
