...
# ---------------------------------------------------------------------------
def selftest() -> bool:
    deny = ({"chase.com", "irs.gov"}, ["*.gov", "myhealth*"])
    ...
    add("inbox row from denylisted sender_domain → refuse",
        lambda: guard("event", {"title": "t", "kind": "market", "source": "inbox_miner:market",
                                "metadata": {"sender_domain": "alerts.chase.com"}}) if not domain_denylisted("alerts.chase.com", deny) else (_ for _ in ()).throw(PIIViolation("tier 0")), True)
    ...
    add("denylist: subdomain suffix + glob", lambda: (_ for _ in ()).throw(PIIViolation("x")) if (domain_denylisted("alerts.chase.com", deny) and domain_denylisted("portal.irs.gov", deny) and domain_denylisted("myhealthplus.com", deny) and not domain_denylisted("veris.ai", deny)) else None, True)
