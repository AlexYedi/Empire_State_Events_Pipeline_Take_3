...
- **Financial:** `chase.com`, `ramp.com` *(also a target)*, `docusign` *(bare)*
...
"""
_INBOX_FIXTURE = [
    {"thread_id": "t1", "from": "Chase Alerts <no-reply@alerts.chase.com>", "subject": "Your statement", "labels": ["Inbox"], "snippet": "balance $4,210 acct ending 5531"},
    {"thread_id": "t2", "from": "hello@alerts.hims.com", "subject": "Refill", "labels": ["Inbox"]},
...
def selftest() -> bool:
    ...
    ck("parser: domains/globs/senders/labels parsed; bare word ignored",
       deny.domains == {"chase.com", "ramp.com"} and set(deny.globs) == {"myhealth*", "*insurance*", "*.gov"}
       and deny.senders == {"hello@alerts.hims.com", "blast@coldoutbound.io"} and deny.labels == ["Me", "Me/Health", "Me/Personal Finance", "Job Hunting"]
    ...
    ck("Stage A: subdomain (alerts.chase.com) skipped as deny:domain", any(s["thread_id"] == "t1" and s["reason"] == "deny:domain" for s in a["skipped"]))
    ...
    out = json.dumps(a)
    ck("Stage A: NO denylisted address/domain in the output", not any(x in out for x in ("chase.com", "hims.com", "myhealthplus", "ramp.com", "coldoutbound", "state.ny.gov", "somecare", "bigco")))
