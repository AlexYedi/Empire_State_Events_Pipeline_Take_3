...
## Substrate status, round 3 (2026-09-18, late) — claim backfill complete

- `NOTE` **Every post-event brief is now in the substrate: 20 events · 614 claims (607 approved · 7 held do-not-publish) · 264 speaker/company links · 13 brief documents.** Inherited approval (ratified 2026-09-18) applied via `stage-claims --approve`. Includes 5 briefs the original 14-list missed (Fin x Clay 6/2 · 2x AI 6/4 · NYC AI Demos #10 6/24 · GTM Eng NYC 6/30 · RevGenius IRL 8/12). Every run exited 0; re-runs match, never duplicate.
- `NOTE` **3 attended events were missing from the graph** (AI Demos #10 · MCP Release Party · RevGenius IRL) — created via `ensure-event` with brief-derived rosters (HIGH-confidence, full names only; first-name-only people like "Tristan"/"Caroline" left out). The other 10 older events already existed as `attended` rows from the one-off migration.
- `NOTE` **Parser v3 (selftest 46/46)** — briefs drifted in 8 more ways, all now handled: numbered headings (`## 6. Pro-Tips`, `## 2 · The Thesis`) · bold sub-labels under a catch-all heading (`**Pro-Tips**`, `**Hot-Takes**`) + inline labels (`**Pitfalls** — a · b · c`) · `Gotchas & Practitioner Playbook`, `Top Takeaways`, `Ranked Insights`, `Headline insight` aliases · blockquoted theses · surname attribution (`(Sow)`) with the owner's first name excluded · roster fallback to `Speakers` / `Presenter Map` / `People & Outreach`. Guard: a bold label only opens a section when the name ends the label — `**Thesis evolution across the three rooms:**` (about OTHER events) no longer leaks in. Re-runs now also backfill speaker links for already-staged claims (links are idempotent).
- `NOTE` **Known gaps (not blocking):** (a) the first 7 briefs were staged before `--brief-ref`, so they have claims but no brief `documents` row; (b) old-migration events aren't linked to some seed companies (e.g. June Daytona / Agents Behaving Badly ↔ Daytona/Datadog), so the Continuity Ledger under-counts — an identity job for **S1b (YED-47)**; (c) external stats inside a brief's Stat Bank (Lenny's survey, LeadDev ELR) are tagged `first_hand`, not `web_verified`; (d) Shortlist April funding figures are audio-sourced ("verify before public") and staged at normal confidence.
- `NOTE` **Still open for Alex:** which of the 14 no-evidence events he attended (list in round 1) → those get `attended` rows on a re-run.
- `NOTE` **Attendance resolved (Alex, 2026-09-19):** ATTENDED → Clay RevOps 9/9 · ElevenLabs 9/9 · Attio Formulas 9/9 · Clay livestream 9/16 — now `attended` event rows (+4 events, +12 links). DID NOT attend → Spark 8/26 · AWS Model Eval 8/27 · Remy 9/2 · ER #217 9/8 · AI GTM Connect 9/9 · Sip & Search 9/10 · Attio AE Playbook 9/15 · Beyond Integration 9/15 · Zo 9/15 · Notion Context is King 9/16 — entities only, no event row (recorded `attended:false` in the manifest source). The attendance question is closed. These 4 have no post-event brief, so no claims yet.

## Substrate status, round 4 (2026-09-19) — PR #81 + judge + tail cleanup

- `NOTE` **PR #81 opened** (branch → main; merged `origin/main` first — only conflict was this append-only note, both sides kept).
- `NOTE` **Judge quorum on `substrate.py`: PASS, agree → auto** (`.claude/evals/logs/2026-09-19-substrate-PR81-substrate.jsonl`). Sonnet 0.87 (correctness .85 · completeness .85 · conventions .95 · anti-patterns .90 · diagnostics .75); Gemini 1.0 (first Gemini run lacked the spec → evidence-parity warning → re-run with ADR-10 + review + spine_client).
- `NOTE` **Acted on the judge's findings:** (1) `ensure_event` now REFUSES to create an event without an explicit `kind` — ADR-10 decision 9 enforced in code, not caller discipline; (2) `document_entity` writes counted; (3) re-posted claim links reported as "linked (idempotent)", not "created". **Not acted on, by design:** `claim.quote` unpopulated (safe by omission); `claim_relation` empty (no producer until a consumer needs it); confidentiality is keyword-based (accepted trade-off of parse-only, zero-inference — see false-positive note below).
- `NOTE` **Tail cleared:** 46 topic links DONE (Notion quota reset; 55 topic names pulled; +42 topics, +26 event links; 271 topic links total). First-7 briefs now have `documents` rows — **20/20 briefs stored, all 614 claims point at their brief.** Brief text is NEVER stored (hash + word_count only); `redact_body()` guards any future text storage.
- `NOTE` **Confidentiality filter false positives (safe direction):** "Off the Record" (Rhodium's community name), a planted "confidential" doc in a demo anecdote, a "(Confidential)" slide footer. One Shortlist-April best-practices line (3 items) was skipped because of the first.
- `NOTE` **Still deferred on purpose:** S1b identity (YED-47) — DDL + irreversible merges + needs its own spec; starts after the A/B (YED-172) so the A/B isn't confounded.
