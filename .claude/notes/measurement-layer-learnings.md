...
3. **A `.env` is NOT auto-loaded into a hook's process.** The Stop hook must `source ./.env` itself (guarded). Otherwise `$POSTHOG_PROJECT_KEY` is never seen.
...
## What's live (cycle-1)
- DoD gate (CLAUDE.md `<definition_of_done>`) · 3 signal scanners (`/scan-trends|roles|voices`) · telemetry hook (`build-session-emit.sh` + `build-session-contract.md`) · the judge (`/judge-build`, `.claude/evals/`). PostHog projection pending Alex's `POSTHOG_PROJECT_KEY`.
