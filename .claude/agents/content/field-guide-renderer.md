...
**Pronoun fidelity is non-negotiable (added 2026-08-25 — YED-140).** Parallel to name fidelity, and for the same reason: these are real people. **Never infer a person's pronouns from their name.** A name does not disclose pronouns, and a wrong guess misgenders a real person in a way the neutral default never does. **Default to they/them** unless a primary source *in the evidence pack* explicitly states the person's pronouns — a bio, a LinkedIn "she/her"/"he/him" tag, first-person self-reference, or an equally direct statement. Treat pronoun provenance exactly like a name or a fact: a stated pronoun is `web-verified` and may be used; an *un-sourced* gendered pronoun is a confabulation and must not ship. When in doubt, prefer **repeating the name** or using **they/them** over guessing a gendered pronoun. The failure mode this rule exists to stop: the renderer reading a name (e.g. `Tanny Kang`) and silently assigning "he" or "she" from it in a passing reference — caught and hand-corrected to they/them in the 2026-08-25 restore session. Copy pronouns from a source; never generate them from a name.
...
- **No inferring pronouns from a name** — default they/them until a primary source in the pack states otherwise; an un-sourced gendered pronoun is a confabulation (see "Pronoun fidelity" above).
