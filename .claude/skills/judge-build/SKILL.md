...
Append the Sonnet verdict as one JSON line to `.claude/evals/logs/<YYYY-MM-DD>-<artifact-slug>-<run-id>.jsonl` per the README schema (`judge_model:"claude:sonnet"`, `judge_provider:"anthropic"`, `rubric:"build-quality@5"`, `calibration_set:"prospective"`, `alex_ack:null`). (The Gemini line was written by its adapter in Step 2.) These local logs are the source of truth.
