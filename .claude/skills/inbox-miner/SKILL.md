...
- `search_threads` → `get_thread` (`PLAIN_TEXT`) on hits (bodies allowed here — allowlisted senders only). Hand raw thread text to a **distill subagent** (text in, structured out, NO write tools) to extract candidate signals. This enforces the injection guard (the model that reads attacker-controlled text has no write tools). **It does NOT keep bodies out of the parent:** in v1 the parent fetches them, so they are in its context (corrected 2026-09-19, YED-203). A subagent *can* call `get_thread` itself (verified by a live test 2026-09-19), which would keep bodies out; that optimization was deliberately dropped 2026-09-18 unless scan latency bites (see `scan-inbox.md`).
...
**Subject-company URL wins over the source's URL (ruled 2026-09-18).** When a VC or aggregator newsletter writes about a company, `primary_url` is the **subject company's** own primary source (its announcement, blog, filing), never the VC firm's or newsletter's page about it. If only the VC's page exists, use it and lower `confidence` one step.
