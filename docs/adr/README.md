...
| [ADR-10](ADR-10-knowledge-substrate.md) | The **Knowledge Substrate**: claims first-class and pointing at events (never promoted into them), documents generalized to every artifact, **one producer path** (`substrate.py` → `spine_client`) and **one retrieval path** (`retrieve.py --lens`) | Additive-only DDL first, identity half rehearsed on the twin; no `observed` kind; no MCP DDL; no ranking on usage before ≥20 outcomes — *Proposed 2026-09-18 (stub; body in the substrate worktree)* |
