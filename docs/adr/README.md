...
| [ADR-9](ADR-9-pii-boundary.md) | The **PII boundary**: three tiers (never-enters · private review · spine · public) + HubSpot one-way; **one hard-fail write chokepoint** (`spine_client.guard`) | Email/phone never enter the spine; research-derived contact PII never enters Notion; the hub carries aggregates and public-figure context only, proven by a failing check — *Accepted 2026-09-13 (YED-81)* |
| [ADR-10](ADR-10-knowledge-substrate.md) | The **Knowledge Substrate**: claims first-class and pointing at events (never promoted into them), documents generalized to every artifact, **one producer path** (`substrate.py` → `spine_client`) and **one retrieval path** (`retrieve.py --lens`) | Additive-only DDL first, identity half rehearsed on the twin; no `observed` kind; no MCP DDL; no ranking on usage before ≥20 outcomes — *Proposed 2026-09-18; body + S1a/S2 migrations merged 2026-09-19 (#81) and live on prod after a green twin rehearsal; **Accepted when the A/B (YED-172) has run***, per the ADR's own status line |
